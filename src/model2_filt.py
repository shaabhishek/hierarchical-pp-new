...
class Model2Filter(BaseModel):
    def __init__(self, n_sample, latent_dim=20, marker_dim=31, marker_type='real', hidden_dim=128, time_dim=2,
                 n_cluster=5, x_given_t=False, time_loss='normal', gamma=1., dropout=None, base_intensity=0.,
                 time_influence=0.1):
        super().__init__()
    ...
    def encoder(self, phi_xt, h_t, temp, mask, n_sample):
        ...
        return sample_y[1:, 0, :, :], ret_sample_z[:, :, :], logits_filter_y[:, 0, :, :], (
        ret_mu_z[:, :, :], ret_logvar_z[:, :, :]), sample_pred_y, pred_z
    ...
    def forward(self, marker_seq, time_seq, anneal=1., mask=None, temp=0.5):
        ...
        meta_info = {"marker_nll": marker_loss.detach().cpu(), "time_nll": time_loss.detach(
    ...
    def _forward(self, x, t, temp, mask):
        ...
        posterior_sample_y, posterior_sample_z, posterior_logits_y, (
            posterior_mu_z, posterior_logvar_z), pred_y, pred_z = self.encoder(phi_xt, hidden_seq[:-1, :, :], temp,
                                                                               mask, n_sample=n_sample)
